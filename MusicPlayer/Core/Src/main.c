...
// 歌词结构体：存放一句歌词的时间和文本
typedef struct {
    uint16_t time_sec;  // 把 [00:03.00] 换算成 3 秒
    char text[32];      // 歌词正文 (最多存31个字母)
} Lyric_t;

// 我们最多存 15 句歌词防爆内存
Lyric_t lrc_array[15]; 
uint8_t total_lrc_lines = 0;   // 记录这首歌总共有几句歌词
uint8_t load_lyric_flag = 1;   // 告诉后台任务：该去读文件了！(开机默认读1次)
